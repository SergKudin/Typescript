  ...
  showSubscribe: function (text, bookId) {
    swal({
      ...
      .then(inputValue => {
        ...
        if (!inputValue) {
          swal('Ошибка!', "Нужно что-то написать!", "error");
          throw null;
        }
        ...
        doAjaxQuery('GET',
          ...
          function (res) {
            console.log(res);
            if (res.success) {
              return swal("Отлично!", 'Ваш e-mail ' + inputValue + '\nдобавлен в список ожидания.', 'success');
            }
          }
        );
      })
      .catch(err => {
        if (err) {
          swal("О нет!", "Ошибка обращения к серверу.\nпопробуйте записаться позже.", "error");
        } else {
          swal.stopLoading();
          swal.close();
        }
